...
let pkg_cmd conf no_pager out_fmt pkg_names =
  ...
  | `Long ->
      ...
      in
      let pkgs = Pkg_info.query ~docdir:(Conf.doc_dir conf)
...
let show_cmd conf no_pager out_fmt show_empty field pkg_names =
  ...
  in
  let infos = Pkg_info.query ~docdir:(Conf.doc_dir conf)
    ~esy_mode:(Conf.esy_mode conf) pkgs in
...
let conf =
  ...
  in
  let conf
      b0_cache_dir b0_log_file cache_dir doc_dir esy_mode jobs lib_dir log_level
  ...
  Term.(const conf $ b0_cache_dir $ b0_log_file $ cache_dir $ doc_dir $ jobs $
        lib_dir $ log_level $ odoc_theme $ esy_mode $ share_dir $ tty_cap)
